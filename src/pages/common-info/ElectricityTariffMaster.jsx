import {
  CommonFunction, Auto_Label_Text_Set,
  TitleBar, Auto_SearchDropDown, DropDownItemGetter,
  Auto_AgGrid,Auto_Button_Add_AGgrid, Auto_Button_Delete_AGgrid, Auto_Button_Save_AGgrid, Auto_Button_Search_AGgrid,
  Auto_Spliter, Auto_Button_Export_Excel, Auto_Button_Import_Excel,
...
const ElectricityTariffMaster = ({
  ...
  const primaryKeys_g1 = ["plantcode", "tariffid"]; // 그리드 의 기본 키  
  const primaryKeys_g2 = ["plantcode", "tariffid", "seq"]; // 그리드 2의 기본 키 
  const [ enterSearch, setEnterSearch ] = useState(false);  // 엔터 키로 검색하기 위한 변수
  const [isModifyAllowed, setIsModifyAllowed] = useState(true);
  const [isAddedTariff, setIsAddedTariff] = useState(false);  // 요금 추가 직후 여부
  const addedTariffData = useRef({ plantcode: "", tariffid: "" }); // 추가된 요금 정보
  ...
  const searchinfo = useMemo(
    () => ({
      address: "baseinfo/electricitytariff-lr",
      params: {
  ...
  const [dropdownData, setDropdownData] = useState({
    ...
    applytype: { items: [], mappings: {} },
    season: { items: [], mappings: {} },
  ...
  useEffect(() => {
    const loadDropdownData = async () => {
      try {
        const [plantcodeAll, plantcodeRequired, applytypeAll, applytypeRequired, seasonAll, timezoneAll, unitcodeAll] =
          await Promise.all([
            ...
            DropDownItemGetter(apiUrl, { param1: "common", param2: "1000", param3: "applytype" }),
            DropDownItemGetter(apiUrl, { param1: "common", param2: "1000", param3: "applytype", param4: "X"}),
            DropDownItemGetter(apiUrl, { param1: "common", param2: "1000", param3: "season" }),
            DropDownItemGetter(apiUrl, { param1: "common", param2: "1000", param3: "timezone" }),
            DropDownItemGetter(apiUrl, { param1: "common", param2: "1000", param3: "unitcode" }),
          ]);
        ...
        setDropdownData({
          ...
          applytype: { items: applytypeAll, mappings: CommonFunction.convertData(applytypeRequired) },
          season: { items: seasonAll, mappings: CommonFunction.convertData(seasonAll) },
  ...
  const grid1KeyData = useRef({ plantcode: "", tariffid: "", loadid: "" }); // 그리드1 에서 선택한 사업장 과 요금id, 부하id
  ...
  const columnDefsSub = useMemo(
    () => [
      ...
      { field: "tariffid", headerName: "요금ID", editable: false, hide: true },
      { field: "seq", headerName: "Seq", editable: false, hide: true },
      { field: "loadid", headerName: "부하ID", editable: false, hide: true, defaultValue: grid1KeyData.current.loadid},
      { field: "baseprice",  headerName: "기본요금", cellDataType: "number", editable: false, hide: true},
      { field: "season",
        headerName: "계절",
        cellEditor: "agSelectCellEditor",
        cellEditorParams: { values: Object.keys(dropdownData.season.mappings) },
        editable: true,
        // 드롭다운 세팅
        valueParser: (params) => params.newValue,
        valueFormatter: (params) => dropdownData.season.mappings[params.value],
        minWidth: 100,
      },
      {
        headerName: '경부하 요금',
        children: [
          { field: "lo_t", headerName: "전력요금", editable: true, cellDataType: "number", cellClass: 'text-right',
            valueFormatter: params => {
              const value = params.value;
              if (value == null || value === "") return "";
              return value.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ',');
          ...
          { field: "lo_d", headerName: "할인", editable: true, cellDataType: "number", cellClass: 'text-right',
            valueFormatter: params => {
              const value = params.value;
              if (value == null || value === "") return "";
              return value.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ',');
      ...
      },
      {
        headerName: '중간부하 요금',
        children: [
          { field: "mi_t", headerName: "전력요금", editable: true, cellDataType: "number", cellClass: 'text-right',
            valueFormatter: params => {
              const value = params.value;
              if (value == null || value === "") return "";
              return value.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ',');
          ...
          { field: "mi_d", headerName: "할인", editable: true, cellDataType: "number", cellClass: 'text-right',
            valueFormatter: params => {
              const value = params.value;
              if (value == null || value === "") return "";
              return value.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ',');
      ...
      },
      {
        headerName: '최대부하 요금',
        children: [
          { field: "mx_t", headerName: "전력요금", editable: true, cellDataType: "number", cellClass: 'text-right',
            valueFormatter: params => {
              const value = params.value;
              if (value == null || value === "") return "";
              return value.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ',');
          ...
          { field: "mx_d", headerName: "할인", editable: true, cellDataType: "number", cellClass: 'text-right',
            valueFormatter: params => {
              const value = params.value;
              if (value == null || value === "") return "";
              return value.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ',');
    ...
    [dropdownData, grid1KeyData]
  );

  const searchinfosub = useMemo(() => ({
    address: "baseinfo/electricitytariff-rr",
    params: {
      plantcode: grid1KeyData.current.plantcode ?? "",
      tariffid: grid1KeyData.current.tariffid ?? "",
    },
  }), [searchParams]); // grid1KeyData는 ref라 의존성에 넣을 필요 없음

  const tmpSelectedData = useRef({ plantcode: "", tariffid: "", loadid: "" }); // 그리드1 에서 선택한 사업장 과 요금id, 부하id 임시 저장
  ...
  const afterMainGridSelect = async (event) => {
    ...
    if (addSubData.length !== 0 || filteredData.length > 0) {
      ...
      tmpSelectedData.current.tariffid = selectedRows[0].tariffid;
    ...
    grid1KeyData.current.tariffid = selectedRows[0].tariffid;
  ...
  const rowFocusing = async () => {
    if (gridData.length > 0 && gridRef.current.api) {
      ...
      if (isAddedTariff && addedTariffData.current.plantcode != "") {
        firstRowIndex = gridData.length;  // 초기값은 마지막 데이터
        // 저장 후 grid1KeyData에 넣어놨던 데이터와 같은 행 찾기
        gridRef.current.api.forEachNode((node) => {
          if (node.data["plantcode"] === addedTariffData.current.plantcode
            && node.data["tariffid"] === addedTariffData.current.tariffid) {
            firstRowIndex = node.rowIndex;  // 또는 node.rowIndex
          }
        });
        setIsAddedTariff(false);
        addedTariffData.current.plantcode = "";
        addedTariffData.current.tariffid = "";
      }
      ...
      grid1KeyData.current.tariffid = gridData[firstRowIndex].tariffid;
  ...
  useEffect(() => {
    ...
    matchedIndex = gridData.findIndex(
      (item) =>
        ...
        item.tariffid === grid1KeyData.current.tariffid
    );

    if (matchedIndex === -1) {
      // 일치하는 항목이 없을 경우 gridData[0]을 사용
      grid1KeyData.current.plantcode = gridData[0].plantcode;
      grid1KeyData.current.tariffid = gridData[0].tariffid;
  ...
  // 요금 추가 버튼 이벤트
  const onClickAddTariff = async () => {
    setIsModalOpen(true);
  };

  // 요금 추가 예 Modal Confirm 함수
  const handleConfirm = async () => {
    ...
    const url = `${apiUrl}baseinfo/electricitytariff-lc`;
    ...
    if (response.data.jhedher.status == "S") {
      addedTariffData.current.plantcode = response.data.jbody.plantcode;
      addedTariffData.current.tariffid = response.data.jbody.tariffid;
      setIsAddedTariff(true); // 부하 추가 직후
      setExcuteSuccesAndSearch(prev => !prev);  // 재조회
    }
    setIsModalOpen(false);
  };

  // 요금 추가 아니오 Modal Cancel 함수
  const handleCancel = () => {
    setIsModalOpen(false);
  };
  ...
  const handleConfirm2 = async () => {
    ...
    grid1KeyData.current.tariffid = tmpSelectedData.current.tariffid;
  ...
  return (
    ...
    {isModalOpen2 && (
      <Auto_MessageModal
        ...
        title="WARNING" // title
        message={"수정 중인 내용은 저장되지 않습니다.\n요금 상세 내용을 조회하시겠습니까?"} // 메시지
        headericon="failed"
      />
    )}
    <div className="space-y-5">
      <TitleBar title="공급 요금 단가 등록" />
      <Card noborder>
                ...
                <div className="flex justify-between items-start mb-5">
                  ...
                  <div className="flex space-x-2 justify-end">
                    ...
                    <Auto_Button_Save_AGgrid
                      ...
                      modifiedMainData={[...gridData, ...addData]}  // 좌측 그리드의 데이터도 같이 전달
                      modifyAddress="baseinfo/electricitytariff-cud"
...
export default ElectricityTariffMaster;