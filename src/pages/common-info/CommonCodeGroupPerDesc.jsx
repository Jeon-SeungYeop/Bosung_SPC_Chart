...
const CommonCodeGroupPerDesc = () => {
  ...
  const primaryKeys_g2 = ["plantcode", "groupid", "codeid"]; // 그리드 2의 기본 키
  ...
  const searchinfo = useMemo(
    () => ({
      address: "baseinfo/commoncodedesc-r1",
      params: {
        plantcode: searchParams.plantcode?.value ?? "",
        groupid: searchParams.groupid ?? "",
        groupname: searchParams.groupname ?? "",
        desccode: searchParams.desccode ?? "",
        descname: searchParams.descname ?? "",
  ...
  const columnDefs = useMemo(
    () => [
      ...
      { field: "groupid", headerName: "그룹ID", editable: false, minWidth: 100, },
      { field: "groupname", headerName: "그룹명", editable: false, minWidth: 150, },
    ],
    [dropdownData]
  );
  ...
  const columnDefsSub = useMemo(
    () => [
      ...
      { field: "codeid", headerName: "상세코드", editable: (params) => (params.data?.rowstatus === "C" || params.data?.rowstatus === "P"), minWidth: 100,},
      { field: "codename", headerName: "상세코드명", editable: true, minWidth: 120, },
      { field: "remark", headerName: "비고", editable: true, minWidth: 100, },
      { field: "real1", headerName: "관련1", editable: true, minWidth: 80, },
      { field: "real2", headerName: "관련2", editable: true, minWidth: 80, },
      { field: "real3", headerName: "관련3", editable: true, minWidth: 80, },
      { field: "real4", headerName: "관련4", editable: true, minWidth: 80, },
      { field: "real5", headerName: "관련5", editable: true, minWidth: 80, },
      { 
        field: "useflag", 
        headerName: "사용여부", 
        editable: false, 
    ...
    await CommonFunction.fetchAndSetGridData({
      ...
      searchinfo: {
        ...
        params: {
          ...
          groupid: grid1KeyData.current.groupid ?? "",
        },
      },
  ...
  const searchinfosub = useMemo(() => ({
    address: "baseinfo/commoncodedesc-r2",
    params: {
      plantcode: grid1KeyData.current.plantcode ?? "",
      groupid: grid1KeyData.current.groupid ?? "",
      desccode: searchParams.desccode ?? "",
  ...
  useEffect(() => {
    ...
    matchedIndex = gridData.findIndex(
      (item) =>
        ...
        item.groupid === grid1KeyData.current.groupid
    );

    if (matchedIndex === -1) {
      // 일치하는 항목이 없을 경우 gridData[0]을 사용
      grid1KeyData.current.plantcode = gridData[0].plantcode;
      grid1KeyData.current.groupid = gridData[0].groupid;
      matchedIndex = 0;
    } 
  ...
  return (
    <div className="space-y-5">
      <TitleBar title="공통코드 그룹 별 상세 내역 마스터" />
      <Card noborder>
          ...
          <div className="flex flex-col gap-y-1">
            <div className="flex flex-wrap gap-x-24 items-end gap-y-1">
              <Auto_SearchDropDown
                ...
                labelSpacing="-mr-1"
              />
              <Auto_Label_Text_Set label="그룹ID" value={searchParams.groupid} onChange={(e) => updateSearchParams("groupid", e.target.value)} labelSpacing="-mr-3" setEnterSearch={setEnterSearch}/>
              <Auto_Label_Text_Set label="그룹명" value={searchParams.groupname} onChange={(e) => updateSearchParams("groupname", e.target.value)} labelSpacing="-mr-3" setEnterSearch={setEnterSearch}/>
            </div>
            <div className="flex flex-wrap gap-x-24 items-center gap-y-1">
              <Auto_Label_Text_Set label="상세코드" value={searchParams.desccode} onChange={(e) => updateSearchParams("desccode", e.target.value)} labelSpacing="-mr-3" setEnterSearch={setEnterSearch}/>
              <Auto_Label_Text_Set label="상세코드명" value={searchParams.descname} onChange={(e) => updateSearchParams("descname", e.target.value)} labelSpacing="-mr-3" setEnterSearch={setEnterSearch}/>
              <Auto_Radio_Useflag
                ...
                <div className="flex justify-between items-start mb-5">
                  ...
                  <div className="flex space-x-2 justify-end">
                    ...
                    <Auto_Button_Save_AGgrid
                      ...
                      modifyAddress="baseinfo/commoncodedesc-cud"
...
export default CommonCodeGroupPerDesc;